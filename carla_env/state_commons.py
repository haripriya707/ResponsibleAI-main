...
def create_encode_state_fn(vae, measurements_to_include):
    ...
    measure_flags = ["steer" in measurements_to_include,
                     ...
                     "traffic_light_dist" in measurements_to_include]
    ...
    def create_observation_space():
        ...
        if measure_flags[11]: observation_space['traffic_light_state'] = gym.spaces.Box(low=0, high=1, shape=(3,), dtype=np.float32)
        if measure_flags[12]: observation_space['traffic_light_dist'] = gym.spaces.Box(low=0, high=200, shape=(1,), dtype=np.float32)
    ...
    def encode_state(env):
        ...
        if measure_flags[11]:
            # Simple One-hot
            s = env.tl_state  # 0: Red, 1: Yellow, 2: Green
            one_hot = np.zeros(3, dtype=np.float32)
            if 0 <= s <= 2:
                one_hot[s] = 1.0
            encoded_state['traffic_light_state'] = one_hot
        if measure_flags[12]:
            encoded_state['traffic_light_dist'] = [env.tl_dist]
