...
class Lidar(CarlaActorBase):
    ...
    def process_lidar_input(weak_self, raw):
        ...
        if callable(self.on_recv_image):
            ...
            # Clip lidar data to be within array bounds
            lidar_data[:, 0] = np.clip(lidar_data[:, 0], 0, self._width - 1)
            lidar_data[:, 1] = np.clip(lidar_data[:, 1], 0, self._height - 1)
            
            # Convert to integers after clipping
            lidar_data = lidar_data.astype(np.int32)
            
            # Filter out any points that might still be out of bounds (edge case)
            valid_mask = (lidar_data[:, 0] >= 0) & (lidar_data[:, 0] < self._width) & \
                        (lidar_data[:, 1] >= 0) & (lidar_data[:, 1] < self._height)
...
class ObstacleDetector:
    def __init__(self, world, vehicle, distance=20.0, hit_radius=1.0, transform=None, on_detect=None):
        """
        Initializes the obstacle detection sensor.
        
        :param world: The CARLA simulation world.
        :param vehicle: The vehicle actor to attach the sensor to.
        :param distance: The detection range of the sensor (meters).
        :param hit_radius: The width of the sensor's detection zone (meters).
        :param transform: The sensor's mounting position relative to the vehicle.
        :param on_detect: Callback function for processing obstacle detection events.
        """
        self.world = world
        self.vehicle = vehicle
        self.on_detect = on_detect  # Callback function when an obstacle is detected

        # Set sensor position (default: front bumper)
        if transform is None:
            transform = carla.Transform(carla.Location(x=2.5, y=0.0, z=1.0))  # Adjust as needed

        # Load the obstacle detection blueprint
        blueprint = world.get_blueprint_library().find('sensor.other.obstacle')
        blueprint.set_attribute("distance", str(distance))  # Max detection range
        blueprint.set_attribute("hit_radius", str(hit_radius))  # Sensitivity

        # Spawn the sensor
        self.sensor = world.spawn_actor(blueprint, transform, attach_to=vehicle.get_carla_actor(), attachment_type=carla.AttachmentType.Rigid)
        
        # Attach event listener
        weak_self = weakref.ref(self)
        self.sensor.listen(lambda event: ObstacleDetector.process_obstacle_event(weak_self, event))
        

    @staticmethod
    def process_obstacle_event(weak_self, event):
        """
        Processes obstacle detection event and computes the distance & TTC.
        """
        self = weak_self()
        if not self:
            return

        distance = event.distance  # Distance to detected obstacle

        # Compute Time to Collision (TTC)
        ttc = self.calculate_ttc(distance)

        # Call the callback function (if provided)
        if self.on_detect:
            self.on_detect(ttc, "obstacle")  # Pass both ttc and obstacle type

    def calculate_ttc(self, distance):
        """
        Calculates Time to Collision (TTC) based on vehicle speed.
        
        :param distance: Distance to the nearest obstacle.
        ...
        if speed <= 0:
            print("Vehicle is stationary, TTC is infinite.")
            return float('inf')
    ...
    def destroy(self):
        """Destroys the sensor to free up resources."""
        if self.sensor is not None:
            self.sensor.destroy()
            print("Obstacle detector destroyed.")
# ===============================================================================
# Camera
# ===============================================================================
